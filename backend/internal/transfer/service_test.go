...
func newReceiverService(t *testing.T, senderID string) (*Service, *SQLiteStore) {
	...
	identity := services.Identity{ID: uuid.NewString(), Name: "Receiver", Type: "desktop", Platform: "test"}
...
func TestInboundTransferApprovalOutOfOrderChunksVerificationAndFinalize(t *testing.T) {
	...
	received, err := service.ReceiveOffer(context.Background(), offer, "127.0.0.1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestOfferRejectsUntrustedSpoofedAndUnsupportedPeers(t *testing.T) {
	...
	if _, err := service.ReceiveOffer(context.Background(), offer, "127.0.0.1"); !errors.Is(err, ErrUntrustedDevice) {
		t.Fatalf("expected untrusted error, got %v", err)
	}
	offer.DeviceID = senderID
	if _, err := service.ReceiveOffer(context.Background(), offer, "192.168.1.10"); !errors.Is(err, ErrUntrustedDevice) {
...
func TestSenderAndReceiverProtocolStreamsFolderWithCompression(t *testing.T) {
	...
	handler := http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case request.Method == http.MethodPost && request.URL.Path == "/v1/transfers/offers":
			...
			if err == nil {
				_, err = receiver.ReceiveOffer(request.Context(), offer, "127.0.0.1")
			}
			if err == nil {
