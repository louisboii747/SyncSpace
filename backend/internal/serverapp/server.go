...
// Run starts SyncSpace's complete local and peer runtime and blocks until ctx
// is cancelled or a listener fails. Desktop and headless entry points share
// this composition root so release builds do not need a companion executable.
func Run(ctx context.Context, logger *slog.Logger, logBuffer *diagnostics.LogBuffer, appVersion string) error {
	config, err := loadConfig(appVersion)
	if err != nil {
		return err
	}
...
func loadConfig(buildVersion string) (serverConfig, error) {
	port := defaultPort
